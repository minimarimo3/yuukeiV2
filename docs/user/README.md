...
一方、現在の`runtime: "process"` Extensionは、信頼したローカルコードとして実行されます。追加時にmanifestの権限を確認し、許可しなければロードされません。許可したmanifestは固定され、後から権限を変えるには削除と再追加が必要です。ただし、v1ではOSレベルのsandboxを保証しません。知らない作者のExtensionをインストールする前に、配布元、ソース、実行コマンドを確認してください。
