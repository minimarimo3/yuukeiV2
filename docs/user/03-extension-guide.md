...
設定画面のExtension欄からフォルダを選びます。Yuukeiはmanifestに宣言された権限と、processを信頼済みローカルコードとして実行する旨を表示します。許可しなければExtensionはコピーもロードもされません。許可したmanifestは固定され、後から権限を変更できません。manifestを変更したい場合は、Extensionを削除してからもう一度追加し、改めて内容を確認します。許可後、Yuukeiはフォルダを次へコピーします。
...
4. 表示された権限と信頼済みコード実行を確認し、`yes`で許可
5. 一覧に表示されたExtensionを有効化
6. `4`で話しかけ、加工後セリフを確認

インストールは参照ではなくコピーです。元の作業フォルダを編集しただけでは、インストール済みコピーは更新されません。開発中にmanifestを変更した場合も、導入済みExtensionをいったん削除してから再追加し、権限をもう一度確認してください。
