...
- [ ] VOICEVOX読み上げ【中物・要判断】— v1に含めるか、v1.1へ回すかをM1着手時に決める。provider不在時は無音継続なので後回しにしてもリリースは阻害しない
...
- [ ] default packの拡充 — M1の新シグナル(hitSurface、不在/復帰、選択肢)への反応台本、2住人の掛け合いを最低限
...
- [ ] 観測の種類ごとのON/OFF設定と、何を記録しているかの説明画面
...
- [ ] 対象OSの決定と実機確認 — macOS(署名+notarization、`macOSPrivateApi` の審査影響確認)/ Windows(インストーラ、透過ウィンドウ動作)。伺か文化圏はWindows比重が高い点を考慮
- [ ] ログイン時自動起動オプション(住人は常駐してこそ)
- [ ] 更新手段 — tauri-plugin-updaterか、まずは手動更新告知か。v1は手動+更新通知で十分
- [ ] ライセンス選定、バージョニング方針、CHANGELOG開始
...
埋め込み検索フェーズ2、デスクトップ観測、Extension配布エコシステム、モバイルSurface、event logタイムラインビューア。いずれも本書の中距離・遠距離のまま維持し、v1.xで検討する。

### リリース判定基準(Definition of Done)

- 開発ツールなしのクリーンなマシンで、インストーラから起動して住人が現れる
