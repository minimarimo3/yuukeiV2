...
- [x] actorウィンドウのキャラ外周(クリック可能シルエットの外側)に白い縁取りを常時表示。CSS/SVGフィルタ(feMorphology dilate→白→元画像合成)を `.actor-canvas` に適用。MToonアウトラインは内側輪郭にも線が出るため不採用(ユーザー確認済)。ヒットテスト(readPixels)はフィルタ適用前のバッファを読むためクリック領域は不変。仕様は02のSurface Client節。実機GUIでの見た目確認は未実施(ユーザー確認待ち)
