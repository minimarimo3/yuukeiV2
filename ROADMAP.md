...
- [ ] 吹き出しを住人ごとに同時1個へ制限し、同一シーン内の連続セリフは読み時間(文字数×90ms、2.5〜9秒)で順送り、別イベントのセリフが来たら残りキューを破棄して即置き換え(ハイブリッド方式、2026-07-12ユーザー確定)。仕様は03のdialogue.say節と02のDevice Host節
- [ ] 未解決の選択肢付き吹き出しは置き換え保護。あわせて選択肢単独吹き出しのtimeoutが30秒へクランプされる潜在バグ(仕様上限600秒)を修正
- [ ] 未使用レガシーの削除: `ActorBubbleLayer.tsx` / `actorBubbleLayout.ts` と両テスト(`ActorSnapshot.bubble` fieldは互換のため残す)
