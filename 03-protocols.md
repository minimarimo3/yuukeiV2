...
v1の実装は「再生開始のみ同期」で妥協する。Resident Homeは `dialogue.say` を即時配信し、`speech.synthesis` routeが登録されている場合だけ並行して合成を依頼する。合成入力は `text`、`speakerId`(canonical actor ID)、`displayCommandId` で、出力は音声本体ではなく参照(`audioPath` などのローカル参照)と `durationMs`。成功時はResident Homeが `audio.play`(payload: `audioPath`、`durationMs`、causalityの `sourceCommandId` に元の `dialogue.say`)を発行し、Device Hostが再生する。新しい `audio.play` が来たら再生中の音声は中断してよい。provider不在・timeout・失敗時は何も発行せず、テキスト表示だけが続く。字幕同期やvisemeは将来の拡張に残す。
