...
pub fn is_bubble_surface_window_label(label: &str) -> bool {
    label.starts_with(BUBBLE_SURFACE_LABEL_PREFIX)
}

pub(super) fn is_legacy_stage_overlay_label(label: &str) -> bool {
    label.starts_with(LEGACY_STAGE_OVERLAY_LABEL_PREFIX)
}
...
pub fn bubble_surface_window_label(actor_id: &str) -> String {
    let mut label = String::from(BUBBLE_SURFACE_LABEL_PREFIX);
    for byte in actor_id.as_bytes() {
        label.push_str(&format!("{byte:02x}"));
    }
    label
...
pub(super) fn create_bubble_surface_window(
    app: &AppHandle,
    actor_id: &str,
    initial_position: &StageRect,
) -> Result<(), String> {
    let label = bubble_surface_window_label(actor_id);
    crate::track_surface_window_loading(app, &label)?;
    let window = WebviewWindowBuilder::new(app, &label, bubble_surface_url(actor_id))
        .title("")
        // The WebView needs a useful viewport to measure its content, but it
        // remains hidden until the measured, tightly bounded placement arrives.
        .inner_size(392.0, 400.0)
...
///
/// New actor and bubble windows are built hidden, passed through this function,
