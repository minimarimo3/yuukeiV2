...
- あなたの仕事は **設計・仕様書作成・Codexへのタスク委任・成果物の検証・コミット**。コード実装はローカルCodex(gpt-5.6)へ委任するのがこのプロジェクトの確定方針(コスト最適化)。手順は `/codex` スキル参照。
...
- 委任は `codex@openai-codex` プラグインの `/codex:rescue`(内部で `codex:codex-rescue` サブエージェント → `codex-companion.mjs task`)。ランタイムは初回コマンドでオンデマンド起動されるので事前起動は不要。難度でモデルを使い分ける(基本=`--model gpt-5.6-terra --effort high`、難物=`gpt-5.6-sol`、単純=`--effort medium`)
- Codexのsandboxは**ネットワーク不可**。新しい依存はあなたが先に `cargo fetch` / `pnpm install` でロックしてから渡す
- 重いターン1回でCodexのレート枠(5時間窓)を使い切ることがある。枯渇したら時間を置き、回復後に `--resume` で追タスクを再投入する(ScheduleWakeupで回復待ち)
- 仕様書はscratchpadに書き、本文をそのまま `/codex:rescue` のタスクテキストとして渡す(リポジトリ外のパスは読めないことがある)。「確定済み設計判断は変更しないこと」「コミットはこちらで行う」を毎回明記する
