...
セリフの吹き出しも舞台の整合性の一部としてDevice Hostが管理する。表示は住人ごとに同時に1個までとし、同一シーン内の連続セリフは読み時間ベースで順送り、別イベントのセリフが来たら残りを破棄して置き換える(表示ルールの詳細は03)。音声再生は吹き出しキューと同期せず現状の後勝ち1本のままとする(完全同期は将来候補)。
