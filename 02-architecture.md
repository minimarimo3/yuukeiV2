...
デスクトップ版のログイン時自動起動は配布buildからだけ新規登録できる。開発buildはフロントエンドdev serverに依存するため登録を拒否するが、過去に作られた開発版の登録は設定画面から解除できる。自動起動エントリには専用引数を付け、手動起動と区別する。Desktop Surfaceのactor windowと全画面overlayはWebViewを非表示で作り、フロントエンドの初期データ取得と描画が完了したready通知を受けてから表示する。readyがtimeoutした場合は生のブラウザエラー画面を表示せず、windowを非表示のままにしてアプリ動作ログへ記録し、トレイのtooltipへ失敗状態を表示する。自動起動時は未完了のonboarding設定画面も自動表示せず、ユーザーがトレイから開いたときに提示する。
