...
つつく・なでるのジェスチャーが起きると、`＠クリック` などで発火するsceneには `入力#actorId`(触られた住人のID)、`入力#hitZoneId`(部位ID)、`入力#hitZoneLabel`(部位の表示名)が渡される。Daihon作者は `条件:（入力#hitZoneId = 「head」 かつ 入力#actorId = 「yuukei」）` のように、部位と住人ごとの反応を通常の条件として書ける。
