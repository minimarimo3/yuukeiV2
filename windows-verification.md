...
- [ ] 自動的に隠すタスクバー: 主画面と副画面のタスクバーを自動非表示にし、各画面端で表示したとき、Yuukeiの住人・舞台overlayより前面に出る。住人・overlayは通常アプリの前面を維持する
- [ ] タスクバーZ順の再確認: 住人をクリックしてフォーカスし、会話composerを開閉し、台本イベントを連続発火した後も、表示した主/副タスクバーが前面に残る
